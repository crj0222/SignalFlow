...
CLASSIFIER_TIMEOUT = float(os.getenv("OPENAI_CLASSIFIER_TIMEOUT_SECONDS", "4"))
...
trade_note should be "Half Size", "Lotto", "Swing", "Day Trade", a slash-combo like "Half Size / Lotto", or null.
...
Price means the option fill/entry/trim/exit price, such as "@ 1.20", "at .95", "paid 1.35", "avg 1.10", "filled 2.40", or a decimal right after the contract.
...
async def classify_alert(content: str) -> Optional[ParsedAlert]:
    """Use AI classification when configured, with parser fallback for local testing."""
    if AI_ENABLED and os.getenv("OPENAI_API_KEY"):
        try:
            parsed = await _classify_with_openai(content)
            return None if _looks_forward_only(content, parsed) else parsed
